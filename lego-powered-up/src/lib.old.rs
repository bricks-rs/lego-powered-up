...
use btleplug::api::{
    bleuuid::uuid_from_u16, Central, Manager as _, Peripheral as _, ScanFilter,
    WriteType,
};
...
pub async fn print_adapter_info(idx: usize, adapter: &Adapter) -> Result<()> {
...
impl PoweredUp {
    ...
    pub async fn with_device(dev: usize) -> Result<Self> {
        let manager = Manager::new().await?;
    ...
    async fn run(&mut self) -> Result<()> {
        let event_rx = self.adapter.write().unwrap().events().await?;
    ...
    pub async fn peripheral(
        &self,
        dev: &PeripheralId,
    ) -> Option<impl Peripheral> {
        self.adapter.write().unwrap().peripheral(dev).await.ok()
    }
    ...
    fn wait_for_hub_filter_timeout_internal(
        ...
        select! {
            ...
            default(timeout) => {
                Err(Error::TimeoutError("Timeout reached".to_string()))
            }
        }
    }
            ...
            if let Ok(evt) = event_rx.recv() {
                ...
                match evt {
                    DeviceDiscovered(dev) => {
                        ...
                        let peripheral =
                            adapter.peripheral(&dev).await.unwrap();
                        debug!(
...
enum HubManagerMessage {
    ...
    Notification(PeripheralId, NotificationMessage),
    SendToHub(PeripheralId, NotificationMessage, Sender<Result<()>>),
    Disconnect(PeripheralId, Sender<Result<()>>),
    GetPort(PeripheralId, Port, Sender<Result<PortController>>),
    GetAttachedIo(PeripheralId, Sender<Result<Vec<ConnectedIo>>>),
}
...
impl HubManager {
    pub fn run(
        ...
        let mut hubs: HashMap<PeripheralId, Box<dyn Hub + Send + Sync>> =
            Default::default();
    ...
    async fn connect_to_hub(
        ...
        let peripheral = adapter.write().unwrap().peripheral(&hub.addr).await?;

        peripheral.connect().await?;
        let chars = peripheral.discover_services().await?;
