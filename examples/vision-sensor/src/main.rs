...
async fn reader(device: &IoDevice, port_id: u8, mode_id: u8, delta:u32, name:String) -> Result<JoinHandle<()>> {
...
async fn vision_to_hub_color(device: &IoDevice, mutex: HubMutex, ) -> Result<JoinHandle<()>> {
    let hubled: IoDevice;
    {
        let lock = mutex.lock().await;
    ...
    Ok(tokio::spawn(async move {
        while let Ok(data) = vision_rx.recv().await {
            println!("Color: {:?} ", data, );
            match data {
                DetectedColor::NoObject => { let _ = hubled.set_hubled_color(Color::Black).await; },
                DetectedColor::Black => { let _ = hubled.set_hubled_color(Color::Black).await; },
                ...
                DetectedColor::Blue => { let _ = hubled.set_hubled_color(Color::Blue).await; },
                // DetectedColor::Color4 => { hubled.set_hubled_color(Color::LightBlue).await; },
                DetectedColor::Green => { let _ = hubled.set_hubled_color(Color::Green).await; },
                // DetectedColor::Color6 => { hubled.set_hubled_color(Color::Green).await; },
                DetectedColor::Yellow => { let _ = hubled.set_hubled_color(Color::Yellow).await; },
                // DetectedColor::Color8 => { hubled.set_hubled_color(Color::Orange).await; },
                DetectedColor::Red => { let _ = hubled.set_hubled_color(Color::Red).await; },
                DetectedColor::White => { let _ = hubled.set_hubled_color(Color::White).await; },
                _ => ()
            }
        }
    }))
...
async fn vision_to_hub_rgb(device: &IoDevice, mutex: HubMutex) -> Result<JoinHandle<()>> {
    let hubled: IoDevice;
    {
        let lock = mutex.lock().await;
        hubled = lock.io_from_kind(IoTypeId::HubLed).await.expect("Can't access Hubled");
    ...
    Ok(tokio::spawn(async move {
        while let Ok(data) = vision_rx.recv().await {
            let _ = hubled.set_hubled_rgb(&[data[0] as u8, data[1] as u8, data[2] as u8  ]).await;
            println!("RGB: {:?} ", data, )
        }
    }))
}